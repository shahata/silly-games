...
class MainController {
  ...
  getStyle(rowIndex, colIndex) {
    ...
    return (rowIndex % BOX_HEIGHT === 0 ? border("top") : "") + (rowIndex % BOX_HEIGHT === BOX_HEIGHT - 1 ? border("bottom") : "") + (colIndex % BOX_WIDTH === 0 ? border("left") : "") + (colIndex % BOX_WIDTH === BOX_WIDTH - 1 ? border("right") : "");
  }
  ...
  getPossibleValues(rowIndex, colIndex) {
    ...
    for (let i = 1; i <= TABLE_WIDTH; i++) {
      if (!this.isInRow(rowIndex, i) && !this.isInCol(colIndex, i) && !this.isInBox(rowIndex, colIndex, i)) {
        arr.push(i);
      }
    }
    return arr;
  ...
  otherOptionInTable(rowIndex, colIndex, value) {
    return this.otherOptionInRow(rowIndex, value) && this.otherOptionInCol(colIndex, value) && this.otherOptionInBox(rowIndex, colIndex, value);
  }
